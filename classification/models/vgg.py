...
class VGG(nn.Module):
    ...
    def get_stage_module(self, stage):
        if stage == 0:
            stage_module = self.block0
        elif stage == 1:
            stage_module = nn.ModuleList([self.pool0, self.block1])
        elif stage == 2:
            stage_module = nn.ModuleList([self.pool1, self.block2])
        elif stage == 3:
            stage_module = nn.ModuleList([self.pool2, self.block3])
        elif stage == 4:
            stage_module = nn.ModuleList([self.pool3, self.block4])
        else:
            raise ValueError
