...
class MobileNetV2(nn.Module):
    ...
    def get_stage_module(self, stage):
        if stage == 0:
            stage_module = nn.ModuleList([self.conv1])
        elif stage == 1:
            stage_module = nn.ModuleList([self.blocks[0], self.blocks[1]])
        elif stage == 2:
            stage_module = nn.ModuleList([self.blocks[2]])
        elif stage == 3:
            stage_module = nn.ModuleList([self.blocks[3], self.blocks[4]])
        elif stage == 4:
            stage_module = nn.ModuleList([self.blocks[5], self.blocks[6], self.conv2])
        else:
            raise ValueError
